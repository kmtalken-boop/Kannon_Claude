...
class TestPlayoffs(unittest.TestCase):
    def setUp(self):
        ...
        self.players_by_id = {p.id: p for plist in self.groups.values() for p in plist}
        self.config = MatchConfig()
        self.rng = random.Random(13)
        self.group_results = run_league_phase(self.groups, self.config, self.rng)

    def test_seeding_produces_eight_unique_qualifiers(self):
        seeds = seed_playoffs(self.group_results, self.rng)
        self.assertEqual(len(seeds), 8)
        self.assertEqual(len(set(seeds)), 8)
        all_qualifiers = {pid for g in self.group_results.values() for pid in g.standings()[:2]}
        self.assertEqual(set(seeds), all_qualifiers)

    def test_bracket_assignment_matches_seed_rule(self):
        result = run_playoffs(self.group_results, self.players_by_id, self.config, self.rng)
        seeds = result.seeds
